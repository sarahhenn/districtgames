...
# =============================================================================
        ...
        for dev in solar:
            for d in days:
                for t in time_steps:
                    ...
                    model.addConstr(heat[dev,d,t] == 
                          eta_th * capacity[dev] * solar_irrad,
                          name="Solar_thermal_"+dev+timetag)
                    model.addConstr(power[dev,d,t] == 
                          eta_el * capacity[dev] * solar_irrad,
        ...
        model.addConstr(soc_nom[dev] == params["rho_w"] * params["c_w"] * 
                        devs[dev]["dT_max"] / 3600000 * capacity[dev], 
                        name="Storage_Volume")
        
        dev = "bat"
        model.addConstr(soc_nom[dev] == capacity[dev] , name="Capacity_"+dev)
        ...
        for d in days:
            for t in time_steps:
                ...
                model.addConstr(p_ch[d,t] <= devs[dev]["P_ch_fix"] + 
                                capacity[dev] * devs[dev]["P_ch_var"],
                                name="P_ch_max"+timetag)
    
                model.addConstr(p_dch[d,t] <= devs[dev]["P_dch_fix"] + 
